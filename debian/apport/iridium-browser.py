...
/usr/share/apport/package-hooks/iridium-browser.py
...
RELATED_PACKAGES = [
    PACKAGE,
    'iridium-browser-l10n',
    'iridium-browser-inspector',
    'iridium-browser-dbg',
    'iridium-codecs-ffmpeg',
    'iridium-codecs-ffmpeg-extra',
    'libvpx0',
    'libgtk2.0-0',
    'nspluginwrapper',
    # various plugins
...
def get_user_profile_dir():
    profiledir = HOME + "/.config/iridium/Default"
...
def add_info(report, userdir=None):
    ...
    for filename in os.listdir('/etc/iridium-browser/customizations'):
        try:
            with open(os.path.join('/etc/iridium-browser/customizations', filename)) as f:
                report['etcconfig/c/'+filename] = "\n".join(f.readlines())
        except IOError:
            report['etcconfig/c/'+filename] = "# could not read"

    try:
        with open('/etc/iridium-browser/default') as f:
